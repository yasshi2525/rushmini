...
class ViewerContainer<T> {
    ...
    public createListener(loadedScene: g.Scene, parent: g.E): ModelChangeListener<T> {
        return {
            onDone: (obj: T) => {
                // モデルと描画物を対応づけておかないと、
...
const createRailViewer = (loadedScene: g.Scene) => {
    ...
    modelListener.railEdge.register(
        railEdge.createListener(loadedScene, panel)
    );

    modelListener.station.register(
        station.createListener(loadedScene, panel)
    );
